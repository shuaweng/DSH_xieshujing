...
const SKILL_SETTINGS_PATH = '.novel/skill-settings.json'
const SKILL_SETTINGS_MAX_BYTES = 64 * 1024
...
export class LocalNovelRepository extends NovelRepository {
  ...
  override async readSkillSettings(
    project: NovelProjectSnapshot,
    signal?: AbortSignal,
  ): Promise<NovelSkillSettings> {
    const target = await this.skillSettingsTarget(project, signal)
    const info = await this.ctx.fs.stat(target, signal)
    if (info === undefined) return { version: 1, disabled: [] }
    if (info.type !== 'file') {
      throw new NovelRepositoryError(
        `novel repository: Skill settings "${target.displayPath}" is not a regular file`,
        'NOVEL_PROJECT_MANIFEST_INVALID',
      )
    }
    const bytes = await this.ctx.fs.readBytes(target, signal, SKILL_SETTINGS_MAX_BYTES)
    let value: unknown
    try {
      value = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(bytes))
    } catch (error: unknown) {
      throw new NovelRepositoryError(
        `novel repository: Skill settings "${target.displayPath}" is not valid UTF-8 JSON`,
        'NOVEL_PROJECT_MANIFEST_INVALID',
        { cause: error },
      )
    }
    return normalizeSkillSettings(value)
  }

  override async replaceSkillSettings(
    project: NovelProjectSnapshot,
    settings: NovelSkillSettings,
    signal?: AbortSignal,
    sandboxPolicy?: SandboxExecutionPolicy,
  ): Promise<NovelSkillSettings> {
    ...
    const text = `${JSON.stringify(normalized, undefined, 2)}\n`
    try {
      ...
      throw new NovelRepositoryError(
        'novel repository: Skill settings changed during replacement',
        'NOVEL_PROJECT_MANIFEST_INVALID',
        { cause: error },
      )
    }
    this.ctx.emit('novel/skill-settings-changed', project.id)
    return normalized
  }
  ...
  private async skillSettingsTarget(
    project: NovelProjectSnapshot,
    signal?: AbortSignal,
  ): Promise<FsTarget> {
    const target = await this.ctx.fs.resolve(SKILL_SETTINGS_PATH, {
      cwd: this.ctx.fs.processPath(project.root),
      ...(signal === undefined ? {} : { signal }),
    })
    if (!this.ctx.fs.contains(project.root, target)) {
      throw new NovelRepositoryError(
        'novel repository: Skill settings path escapes the project root',
        'NOVEL_PROJECT_PATH_ESCAPE',
      )
    }
    return target
...
function normalizeSkillSettings(value: unknown): NovelSkillSettings {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw new NovelRepositoryError('novel repository: Skill settings must be an object', 'NOVEL_PROJECT_MANIFEST_INVALID')
  }
  const record = value as Record<string, unknown>
  if (record['version'] !== 1 || !Array.isArray(record['disabled'])) {
    throw new NovelRepositoryError(
      'novel repository: Skill settings must use version 1 with a disabled array',
      'NOVEL_PROJECT_MANIFEST_INVALID',
    )
  }
  const disabled = record['disabled']
  if (disabled.some(name => typeof name !== 'string' || !SKILL_NAME.test(name))) {
    throw new NovelRepositoryError(
      'novel repository: disabled Skill names must be unique kebab-case strings',
      'NOVEL_PROJECT_MANIFEST_INVALID',
    )
  }
  const normalized = [...new Set(disabled as string[])].sort()
  if (normalized.length !== disabled.length) {
    throw new NovelRepositoryError(
      'novel repository: disabled Skill names must be unique kebab-case strings',
      'NOVEL_PROJECT_MANIFEST_INVALID',
    )
  }
  return { version: 1, disabled: normalized }
}
