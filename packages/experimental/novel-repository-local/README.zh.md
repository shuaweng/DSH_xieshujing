...
- 历史 Schema 版本四会在 ChangeSet 中保存目标 Asset 类型、保留 apply journal、暴露不可变 Revision 摘要，并为每个精确 `(Project, Asset, Revision, kind)` 保存一个通过校验的分析报告信封。持久操作由这个精确的已注册定义解码和物化；提案仍不写文件，审阅仍归 Session 所有。
