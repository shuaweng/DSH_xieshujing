...
- 已注册 Asset 定义选择声明过的内容根和接受的扩展名。每个候选文件都由严格 Frontmatter `novel.type` 通过 `ctx.novelAssetTypes` 分派解析；未知类型、扩展名不匹配、重复 id 和扫描期间发生变化的文件都会以失败关闭处理。内置章节定义要求 `manuscript` 根和严格第一版章节 Frontmatter。
...
- 作者保存会让已注册定义物化并重新解析完整候选字节，再同时使用当前 `FsVersion` 和基础 Revision 拒绝陈旧发布。内置章节定义保留精确 Frontmatter 前缀，并在完整 code-point 边界上校验 UTF-16 范围。
