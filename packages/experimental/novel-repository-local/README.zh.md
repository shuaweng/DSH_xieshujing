...
- 已注册 Asset 定义选择声明过的内容根和接受的扩展名。Markdown Frontmatter 或 YAML 根部 `novel.type` 声明会把每个候选文件分派给 `ctx.novelAssetTypes`；未知类型、扩展名不匹配、重复 id 和扫描期间发生变化的文件都会失败关闭。内置章节定义使用 `manuscript` 下的严格 Markdown；大纲 contribution 使用可选 `planning` 下的严格 YAML。
...
- 作者保存会让已注册定义物化并重新解析完整候选字节，再同时使用当前 `FsVersion` 和基础 Revision 拒绝陈旧发布。章节定义保留精确 Frontmatter 前缀，并在完整 code-point 边界上校验 UTF-16 范围；大纲定义校验稳定 node id 并写回类型化 YAML 内容。
