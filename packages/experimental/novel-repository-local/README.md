...
- Typed creation generates a stable Asset id and safe filename inside the registered content root, validates parent/singleton/depth rules, publishes with `createIfAbsent`, and retains the first Revision. The built-in chapter definition accepts a title and complete manuscript body in one creation; it never requires a separately created empty container. When an empty chapter already exists, its exact-Revision `insert-text` operation can write at offset zero, while `replace-text` remains non-empty. Author saves ask the registered definition to materialize and reparse complete candidate bytes, then use the current `FsVersion` plus base Revision to reject stale publication. Shipped text definitions preserve identity/parent Frontmatter and validate UTF-16 offsets on complete code-point boundaries.
