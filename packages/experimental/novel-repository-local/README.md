...
- Registered Asset definitions select declared content roots, accepted extensions, creation behavior, and semantic-parent rules. Markdown Frontmatter dispatches each candidate through `ctx.novelAssetTypes`; unknown types, extension mismatches, duplicate ids, invalid parents, hierarchy cycles, and files that change during scanning fail closed. The shipped chapter definition uses Markdown under `manuscript`; the planning contribution uses freeform Markdown under optional `planning`.
...
- Typed creation generates a stable Asset id and safe filename inside the registered content root, validates parent/singleton/depth rules, publishes with `createIfAbsent`, and retains the first Revision. Author saves ask the registered definition to materialize and reparse complete candidate bytes, then use the current `FsVersion` plus base Revision to reject stale publication. Shipped text definitions preserve identity/parent Frontmatter and validate UTF-16 ranges on complete code-point boundaries.
