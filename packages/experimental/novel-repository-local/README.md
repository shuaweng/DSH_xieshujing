...
- Registered Asset definitions select declared content roots and accepted extensions. Every candidate's strict Frontmatter `novel.type` dispatches parsing through `ctx.novelAssetTypes`; unknown types, extension mismatches, duplicate ids, and files that change during scanning fail closed. The shipped chapter definition requires the `manuscript` root and strict version-one chapter Frontmatter.
...
- Author saves ask the registered definition to materialize and reparse complete candidate bytes, then use the current `FsVersion` plus base Revision to reject stale publication. The shipped chapter definition preserves the exact Frontmatter prefix and validates UTF-16 ranges on complete code-point boundaries.
