...
- History schema version four stores the target Asset type with ChangeSets, retains the apply journal, exposes immutable Revision summaries, and keeps one validated analysis-report envelope per exact `(Project, Asset, Revision, kind)`. Durable operations are decoded and materialized through that exact registered definition; proposal remains file-read-only and review is Session-owned.
