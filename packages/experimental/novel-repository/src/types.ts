 ...
 *
 * This is a string at remote and persistence boundaries because Typert 0.1.2
 * requires a concrete scalar projection. Repository providers remain the
 * authority that validates the canonical `sha256:` plus 64-lowercase-hex
 * representation before admitting authored or historical values.
 */
...
/** Project-owned activation policy for Skills contributed by the Novel Preset. */
export interface NovelSkillSettings {
  readonly version: 1
  /** Skill names hidden from both model-facing and user-facing invocation surfaces. */
