...
- `listAssets()` reconciles authored files into current catalog rows and applies optional type-specific manifest order, `reorderAssets()` atomically replaces one type's complete stable-ID sequence, `searchAssets()` discovers bounded current exact Revisions through a provider-owned search strategy, `createAsset()` creates one registered type at a provider-owned path, `readAsset()` reads the current or one retained immutable Revision, `listAssetRevisions()` exposes bounded Revision history, `saveAssetContent()` performs a guarded typed-content save, and `captureSelection()` freezes one type-defined semantic selection.
...
- Version one ships `manuscript.chapter` plus separately contributed freeform `planning.outline`, `planning.chapter-outline`, `book.brief`, and `book.style-profile` types, exact `sha256:` content hashes, immutable Revision ancestry, Revision-bound `SelectionRef` values, typed manuscript insertion and exact replacement operations, and durable single-asset ChangeSets carrying their target Asset type.
