...
- `listAssets()` reconciles authored files into current catalog rows and applies optional type-specific manifest order, `reorderAssets()` atomically replaces one type's complete stable-ID sequence, `searchAssets()` discovers bounded current exact Revisions through a provider-owned search strategy, `createAsset()` creates one registered type at a provider-owned path, `readAsset()` reads the current or one retained immutable Revision, `listAssetRevisions()` exposes bounded Revision history, `restoreAssetRevision()` republishes one retained historical snapshot as a new guarded current Revision, `saveAssetContent()` performs a guarded typed-content save, and `captureSelection()` freezes one type-defined semantic selection.
