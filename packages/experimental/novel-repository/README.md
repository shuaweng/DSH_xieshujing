...
- `NovelAssetTypeMap` is merge-extensible. Each matching Host definition owns exact Frontmatter type parsing, model projection, selection validation, save materialization, durable operation decoding, and ChangeSet materialization; registrations are unique and disappear with their caller effect.
- Version-one ships `manuscript.chapter`, exact `sha256:` content hashes, immutable Revision ancestry, Revision-bound `SelectionRef` values, typed `replace-text` operations, and durable single-asset ChangeSets carrying their target Asset type.
...
- **One shipped asset type** — the kernel is extensible, but version one ships only `manuscript.chapter`; outlines, characters, ideas, relations, and view definitions are deferred.
