...
- `listAssets()` 把作者文件协调为当前目录项，`readAsset()` 读取当前或指定的已保留不可变 Revision，`saveAssetContent()` 以版本保护方式保存完整类型化内容，`captureSelection()` 冻结由资产类型定义的语义选区。
- `NovelAssetTypeMap` 可以通过声明合并扩展。每个匹配的 Host 定义拥有精确作者类型解析、模型投影、选区校验、保存物化、持久操作解码和 ChangeSet 物化；注册项必须唯一，并随调用方 effect 释放而移除。
- 第一版内置 `manuscript.chapter`，并由独立包贡献 `planning.outline`；同时提供精确 `sha256:` 内容哈希、不可变 Revision 父链、绑定 Revision 的 `SelectionRef`、类型化 `replace-text` 与 `update-outline-node` 操作，以及携带目标 Asset 类型的持久单资产 ChangeSet。
...
- **内置两种资产类型**：内核已安装 `manuscript.chapter` 与 `planning.outline`；人物、灵感、关系、场景与视图定义均暂缓。
- **每个 ChangeSet 一个操作**：第一版支持一个章节 `replace-text` 或一个大纲 `update-outline-node`，并拒绝自动重定位到较新的 Revision。
