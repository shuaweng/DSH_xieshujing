...
- `NovelAssetTypeMap` 可以通过声明合并扩展。每个匹配的 Host 定义拥有精确 Frontmatter 类型解析、模型投影、选区校验、保存物化、持久操作解码和 ChangeSet 物化；注册项必须唯一，并随调用方 effect 释放而移除。
- 第一版内置 `manuscript.chapter`、精确 `sha256:` 内容哈希、不可变 Revision 父链、绑定 Revision 的 `SelectionRef`、类型化 `replace-text` 操作和携带目标 Asset 类型的持久单资产 ChangeSet。
...
- **只内置一种资产类型**：内核可以扩展，但第一版只内置 `manuscript.chapter`；大纲、人物、灵感、关系与视图定义均暂缓。
