...
export class NovelRepositoryRemote extends TypertRemoteService {
  ...
  async skills(agent: Agent, signal: AbortSignal): Promise<NovelSkillSettingsDescriptor> {
    ...
    const settings = project === undefined
      ? { version: 1 as const, disabled: [] as readonly string[] }
      : await this.ctx.novelRepository.readSkillSettings(project, signal)
    ...
    const skills = snapshot.skills
      ...
      .map(skill => ({
        ...
        enabled: !disabled.has(skill.name) && isModelInvocable(skill),
      }))
  ...
  async replaceSkillSettings(
    ...
    await this.ctx.novelRepository.replaceSkillSettings(
      project,
      { version: 1, disabled: request.disabled },
      signal,
