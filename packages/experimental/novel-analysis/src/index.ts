...
export class NovelAnalysis extends Service {
  ...
  async reviewChapter(
    ...
    const project = await this.resolveProject(agent, signal)
    const skillSettings = await this.ctx.novelRepository.readSkillSettings(project, signal)
    if (skillSettings.disabled.includes('chapter-review')) {
      throw new Error('novel analysis: chapter-review Skill is disabled for this Novel Project')
    }
