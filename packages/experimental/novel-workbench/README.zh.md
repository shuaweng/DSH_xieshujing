---
description: "供人与 Agent 浏览、编辑、审查、恢复并引用类型化 Asset 的 Novel Studio UI。"
...
- 当精确 Session 根目录没有 `novel.yaml` 时，资产浏览器与 Context Tray 会进入中性未初始化状态，不发起 Asset 或上下文工作集请求。响应式品牌启动页以水墨主视觉配合聚焦的作品名与可选简介表单，通过与 Agent 工具相同的 Remote/Repository 操作完成初始化，然后刷新为普通资产界面。次级操作会返回既有、由 DSH 支撑的写书鲸书库，不另造项目选择器；已有但损坏的清单仍会明确报错。
