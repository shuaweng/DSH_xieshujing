...
import { NovelFrame as NovelFrameComponent } from '../src/client/NovelFrame.tsx'
import { WorkbenchToggle as WorkbenchToggleComponent } from '../src/client/WorkbenchToggle.tsx'
import { NovelPresentationCard as NovelPresentationCardComponent } from '../src/client/NovelPresentationCard.tsx'
import { Explorer as ExplorerComponent } from '../src/client/Explorer.tsx'
import { Canvas as CanvasComponent, shortReferenceLabel } from '../src/client/Canvas.tsx'
import { ContextTray as ContextTrayComponent, humanContextLabel } from '../src/client/ContextTray.tsx'
import { NovelHome as NovelHomeComponent, type NovelLibraryBook } from '../src/client/Home.tsx'
import {
...
import { ChangeSetCard as ChangeSetCardComponent, type NovelChangeReview } from '../src/client/ChangeSetCard.tsx'
...
// Production entrypoints keep the framework's full Standard Props contract.
// Isolated component tests intentionally provide only the seats each rendered
// component reads, so erase that assembly-only surface at this test boundary.
type IsolatedComponent = ComponentType<Record<string, unknown>>
const NovelFrame = NovelFrameComponent as unknown as IsolatedComponent
const WorkbenchToggle = WorkbenchToggleComponent as unknown as IsolatedComponent
const NovelPresentationCard = NovelPresentationCardComponent as unknown as IsolatedComponent
const Explorer = ExplorerComponent as unknown as IsolatedComponent
const Canvas = CanvasComponent as unknown as IsolatedComponent
const ContextTray = ContextTrayComponent as unknown as IsolatedComponent
const NovelHome = NovelHomeComponent as unknown as IsolatedComponent
const ChangeSetCard = ChangeSetCardComponent as unknown as IsolatedComponent
...
function useSessions<T>(select: (state: SessionListState) => T): T {
  return select({
    ids: [SID],
    byId: { [SID]: { id: SID, displayTitle: 'Novel', projectionValues: { agentPreset: 'novel-workbench' }, running: false, blank: false, updatedAt: 1 } },
    current: SID,
    phase: 'ready',
    subagentsByParent: {},
    jobsBySession: {},
...
describe('NovelFrame', () => {
  it('renders the elected Novel explorer and canvas inside the shipped shell', () => {
    ...
    expect(workbench.getSnapshot().agentWidth).toBe(426)
    ...
    const host = view.container.querySelector<HTMLElement>('[data-novel-workbench]')!
...
describe('preset-scoped workbench activation', () => {
  it('shows the Composer toggle only for novel-workbench and applies typed Agent presentation metadata', () => {
    ...
    const eligible = render(<WorkbenchToggle
      ...
      session={{ blank: false, promptAttempted: true, nodes: [{}] } as never}
      useWorkbench={hookOf(workbench)}
      toggleWorkbench={() => { workbench.toggle() }} closeWorkbench={() => { workbench.close() }} t={t}
    />)
    ...
    expect(workbench.getSnapshot().visible).toBe(true)
    ...
    const standardSessions = <T,>(select: (state: SessionListState) => T): T => select({
      ids: [SID],
      byId: { [SID]: { id: SID, displayTitle: 'Standard', projectionValues: { agentPreset: 'standard' }, running: false, blank: false, updatedAt: 1 } },
      current: SID, phase: 'ready', subagentsByParent: {}, jobsBySession: {}, currentAddress: undefined,
    })
    eligible.rerender(<WorkbenchToggle
    ...
    />)
    expect(eligible.queryByRole('button')).toBeNull()
    expect(workbench.getSnapshot().visible).toBe(false)
    ...
    render(<NovelPresentationCard
      ...
      present={(intent: 'open-workbench' | 'close-workbench') => {
...
describe('Explorer', () => {
  ...
  it('deletes an Asset after confirmation and removes it from the current catalog', async () => {
    ...
    const view = render(<Explorer
      ...
      open={async (_sid: SessionId, id: string) => id === first.id ? first : second} onRefresh={() => () => {}} t={t}
    />)
    await waitFor(() => { expect(view.getByText('第一章')).toBeTruthy() })
...
describe('Novel workbench stores and browser assembly', () => {
  ...
  it('covers every shared state transition and frame action', () => {
    ...
    expect(frame.getSnapshot()).toMatchObject({ explorerCollapsed: true, page: 'book', requestedAssetId: undefined })
  ...
  it('wires slots, remotes, Composer mentions, reviews, layout, locale, and theme teardown', async () => {
    ...
    const workbenchEntry = slots.entries('shell.overlay').find(entry => entry.component === NovelFrameComponent)!
    ...
    workbenchFace.workbench.setAgentWidth(480)
    expect(workbenchFace.workbench.getSnapshot().agentWidth).toBe(480)
    ...
    const toggleFace = (toggle.inject as () => {
      hooks: { workbench: NovelWorkbenchViewController }
      toggleWorkbench: () => void
      closeWorkbench: () => void
    })()
    expect(toggleFace.hooks.workbench).toBe(workbenchFace.workbench)
    toggleFace.toggleWorkbench()
    expect(workbenchFace.workbench.getSnapshot().visible).toBe(true)
    toggleFace.toggleWorkbench()
    expect(workbenchFace.workbench.getSnapshot().visible).toBe(false)
    ...
    let draft = '前后'
    let draftRev = 0
    const insertReference = vi.fn((reference: ReferenceInsert) => {
      draft = `${draft}@${reference.label} `
      draftRev += 1
      return true
    })
    ctx.provide('conversation', {
      input: { for: () => ({ state: { getSnapshot: () => ({ draft, draftRev }) }, insertReference }) },
    } as never)
    canvasFace.appendReference(SID, selection(), '[新句]')
    expect(draft).toBe('前后@[新句] ')
