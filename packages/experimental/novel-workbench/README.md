...
- The shipped `ui-layout` remains the sole root and layout-service owner. This package contributes its pure `novel` surface through the additive `shell.overlay` seat and declares `novel.explorer` plus `novel.canvas` only beneath that surface. DSH 0.1.2 does not expose a public shell-column API, so a package-local compatibility adapter temporarily maps the rendered shell grid into sidebar, Agent, and workbench tracks while the surface is open, follows native sidebar changes, and removes every marker on teardown. No host source patch, cross-plugin React import, or competing root is required.
...
- When the exact Session root has no `novel.yaml`, the explorer and Context Tray enter a neutral uninitialized state and issue no Asset or context-workset requests. A responsive branded start page pairs an ink-wash visual with a focused title-and-optional-synopsis form, initializes through the same Remote/Repository operation available to the Agent tool, and then refreshes into the normal Asset surface. Its secondary action returns to the existing DSH-backed Xieshujing library instead of inventing another project picker; malformed existing manifests remain explicit errors.
...
- The Novel Agent can call `novel_present` with `open-workbench` or `close-workbench`. Its durable tool-result metadata drives the same browser-local workbench controller as the Composer toggle; ordinary Agent prose never controls layout and presentation never mutates an Asset.
