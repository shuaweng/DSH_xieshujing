...
- The explorer discovers the active Session's Novel Project, presents a quiet draft/chapter hierarchy, opens exact Revision-bound typed Asset documents, and places the active chapter's renderer-derived character count in its bottom status bar.
...
- The shipped manuscript renderer edits a chapter body, captures UTF-16 ranges, counts non-whitespace authored characters, and opts into a centered paper surface with six coordinated workspace/paper/text skins. Compact bottom-right popovers switch skin or typeface and font size; the canvas header presents project/chapter breadcrumbs while technical type and path metadata stay out of the writing surface.
