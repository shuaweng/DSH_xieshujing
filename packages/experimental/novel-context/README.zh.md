...
模型先看到用户的可读消息，随后看到一条 `Referenced novel material` 消息，其中包含精确 Revision 元数据和安全序列化的创作文本。它的第二版 `novel-context` 来源携带确定性 Manifest id、来源和保留模式，因此 Session 回放可以重建同一个上下文切面。

#### Token 影响

只有显式引用与可见的保留工作集会被加入。固定安全框架和 JSON 元数据带来少量开销；配置的引用数和字节上限限制可变部分。
