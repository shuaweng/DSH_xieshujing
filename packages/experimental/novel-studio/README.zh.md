...
模型看到 Novel persona、六个稳定 Novel 工具 Schema、标准 `skill` loader 及其紧凑目录，以及用户显式选择或保留在可见 Context Tray 中的精确资料。只有任务需要时才会加载六个内置写作方法之一。浏览器布局状态永远不会进入模型上下文；Asset 发现只通过 Novel 工具发生。
