...
- 应在现有 base 与 Web App 组合包之后加入本 bundle。它会先插入 Host Asset 类型注册表、独立的自由策划/全书指导 Host/Client contribution 与 `novel-repository-local`，随后加入 context、Remote、独立 Client adapter 和 `novel-workbench`。
...
- 同一 Preset 通过标准按需 `skill` 工具挂载六个包内自包含写作 Skill：大纲/beat 设计、章节执行、文风改写、文风审查、场景推进与对白诊断。Skill 只教授方法，不能扩大 Novel 工具权限。
...
模型看到 Novel persona、六个稳定 Novel 工具 Schema、标准 `skill` loader 及其紧凑目录，以及用户显式选择或保留在可见 Context Tray 中的精确资料。只有任务需要时才会加载六个内置写作方法之一。处理相关大纲、落稿、改写或审查任务时，persona 或已加载方法会显式发现并读取现有 `book.brief` / `book.style-profile` 的当前精确 Revision；这些是可见工具读取，不是隐藏常驻上下文。浏览器布局状态永远不会进入模型上下文。

#### Token 影响

Preset 会加入 persona、六个 Novel Schema、一个 Skill Schema 与紧凑 Skill 目录摘要。只有被调用时，Skill 正文以及显式或保留创作文本才会增加本次请求 token。
...
- **仅首批 Skill**：目前改造六个高频写作与诊断方法。旧的直接文件 Novel Skills 有意不挂载到 Workbench Preset；更多方法会随目标 Asset 类型落地而按 Asset 语义迁移。
