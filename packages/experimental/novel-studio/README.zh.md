...
- 同一 Preset 通过标准按需 `skill` 工具挂载八个包内自包含写作/审稿 Skill：大纲/beat 设计、章节执行、文风改写、文风审查、场景推进、对白诊断、精确 Revision 章节审稿与草稿/定稿偏好提取。Skill 只教授方法，不能扩大 Novel 工具权限。
...
根模型看到 Novel persona、六个稳定 Novel 工具 Schema、标准 `skill` loader 及其紧凑八 Skill 目录，以及用户显式选择或保留在可见 Context Tray 中的精确资料。只有任务需要时才加载方法。处理相关大纲、落稿、改写或审查任务时，persona 或已加载方法会显式发现并读取现有 `book.brief` / `book.style-profile` 的当前精确 Revision；这些是可见工具读取，不是隐藏常驻上下文。审稿与满足条件的显式定稿只把冻结精确资料交给各自专用子 Agent；浏览器布局状态永远不会进入模型上下文。

#### Token 影响

Preset 会加入 persona、六个 Novel Schema、一个 Skill Schema 与紧凑 Skill 目录摘要。只有被调用时，Skill 正文以及显式或保留创作文本才会增加本次请求 token。NOAI 按钮扫描不消耗 token；章节审稿和满足条件的显式定稿各使用一个有边界的子 Agent 请求。
