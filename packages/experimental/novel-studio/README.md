...
The model sees the Novel persona, stable Novel tool schemas, and exact material chosen explicitly or retained in the visible Context Tray. Browser layout state never enters model context; discovery occurs only when the model calls `novel_list` or `novel_search`.

#### Token effect

The Preset adds its persona, a short Novel tool section, and six tool schemas. Explicit and retained authored text adds request-local tokens within the configured context budget.
