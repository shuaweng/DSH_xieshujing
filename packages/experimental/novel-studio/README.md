...
- **First Context Compiler policies only** — task selection is explicit and relation expansion is deterministic. Story State is exact freeform text; semantic retrieval, policy-owned summaries, model-specific token budgeting, and Scene Contracts can extend the compiler seam later without changing V3 frozen-manifest replay.
