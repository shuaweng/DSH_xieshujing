...
The model sees the Novel persona, the stable `novel_list`, `novel_get`, and `novel_propose_changes` schemas, and exact referenced material only when a user sends a canonical workbench mention. Browser layout state is never added to model context; catalog discovery occurs only when the model calls `novel_list`.
