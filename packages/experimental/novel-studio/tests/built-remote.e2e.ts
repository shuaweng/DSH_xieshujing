  ...
  it('discovers through the generated Client namespace and withdraws that namespace', async () => {
    ...
    try {
      ...
      const urls = Object.fromEntries(Object.entries({
        ...
        sessionProjection: 'packages/session/session-projection/lib/index.js',
      ...
      const script = `
        ...
        const { default: SessionProjectionRegistry } = await import(urls.sessionProjection)
        ...
        if (host.get('novelRepositoryRemote') === undefined) {
          const required = ['sandboxPolicy', 'fs', 'novelAssetTypes', 'novelRepository', 'novelContextResolver', 'subagents', 'novelAnalysis']
          const missing = required.filter(key => host.get(key) === undefined)
          throw new Error('built Host did not provide novelRepositoryRemote; missing=' + missing.join(','))
        }
        ...
        if (chapter.ok !== true) {
          throw new Error('chapter read failed: ' + JSON.stringify(chapter) + '; message=' + chapter.error?.message + '; discover=' + JSON.stringify(response) + '; assets=' + JSON.stringify(assets) + '; carrier=' + JSON.stringify(lastCarrierValue))
        }
        ...
        const retainedAfterDisposeMessage = retainedAfterDispose.ok === false
          ? retainedAfterDispose.error.message
          : undefined

        console.log(JSON.stringify({ response, assets, chapter, selection, changeSet, applied, after, withdrawn, retainedAfterDispose, retainedAfterDisposeMessage }))
      ...
      const output = JSON.parse(result.stdout.trim().split('\n').at(-1) ?? '{}') as {
        ...
        retainedAfterDisposeMessage?: string
      }
      ...
      expect(output.retainedAfterDisposeMessage).toContain('is no longer mounted')
