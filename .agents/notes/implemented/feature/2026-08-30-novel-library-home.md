...
The elected Novel surface starts on a browser-local `home` page with three totals, one continue-writing action, and the registered book list. Opening a book uses `ctx.workspaces.connectWorkspace()` and `ctx.sessions.open()` before the existing explorer resolves the requested stable Asset id. No cross-folder content is injected into a model prompt.
...
The homepage sees books across folders only after the author has registered those folders as DSH Workspaces. It performs no whole-disk scan and does not change Session scoping, model context, tools, or project manifests.
