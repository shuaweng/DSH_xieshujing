...
Project initialization is one Repository operation shared by the browser and the Agent tool. It validates a non-empty title, an optional bounded synopsis, and an existing Session root, rejects an existing `novel.yaml` or non-directory `manuscript`/`planning` conflict, creates missing minimal roots through sandboxed create-only writes, and publishes `novel.yaml` last as the activation marker. It never deletes or replaces an existing authored file. The browser represents an absent manifest as a responsive branded split start page: package-owned ink-wash artwork is presentation-only, while the title-and-optional-synopsis form still owns the same initialization request. Its secondary action returns to the existing DSH-backed Xieshujing library rather than adding a second project picker, and narrow workbench widths drop the decorative panel before constraining the form. Asset/context calls stay suppressed until the project becomes ready. `novel_initialize_project` exposes the same operation to the model only after an explicit user request and a one-shot approval; its settled tool card refreshes an open workbench.
...
The branded browser path makes the first project action legible without changing its authority boundary: **Start writing** is the guarded initialization operation, while **Open existing project** is navigation back to the existing library. The artwork intentionally yields at narrow widths and never enters model context.
