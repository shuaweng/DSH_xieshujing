...
Reader presentation is an optional Client renderer capability, not a `manuscript.chapter` branch in the shared canvas. A renderer may supply authored-character counting; the shared full-width bottom bar presents that count plus popovers for a coordinated workspace/paper/text skin and for typeface and font size. The paper fills the canvas between the header and bottom bar, and the workbench viewport—not the editable paper—owns vertical scrolling. Project/chapter breadcrumbs and the editable authored title replace technical type/path metadata on the writing surface. The explorer renders Book-level Manuscript and Outline branches even when empty and may collapse independently. View preferences, explorer visibility, and the Agent/workbench split width are client-only state and never enter Frontmatter, Revision history, Context Manifest, system prompts, or tool schemas. The panel boundary is an accessible pointer and keyboard separator with bounded widths; its current value may be discarded without changing authored content.
