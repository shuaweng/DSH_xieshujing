...
PR1, PR2, the PR3 MVP, the PR4 asset type kernel, the PR5 registry proof, the PR6 presentation slice, the PR7 context workset, the PR8 authoring Skill slice, and the PR9 book-guidance Asset slice are implemented on the feature stack. This note remains proposed because its acceptance criteria intentionally cover later asset types, invalidation events, restart snapshots, finalization/learning, and orchestration that the MVP defers.
...
Browser Consumers gain bounded project-scoped asset list, chapter read, guarded authored save, and selection capture methods. A save retains exact untouched Frontmatter source, allows the registered type to replace its validated authored title scalar and typed content, validates the resulting complete file, requires both the current base Revision and provider-local `FsVersion`, and records the new Revision only after filesystem publication succeeds. A SelectionRef freezes a non-empty UTF-16 range on code-point boundaries over one retained Revision; quote hash, bounded context, and preview are derived from that immutable body. PR2 adds no prompt context, model tool, ChangeSet persistence, or workbench layout.
...
Each Host definition declares its Frontmatter type, accepted content root and extensions, parsed content value, model projection, selection validation, authored-save materialization, durable-operation decoding, and complete ChangeSet materialization. A definition may additionally declare generic semantic-parent rules or `projectSingleton`; the Repository enforces those cardinalities on both authored scans and typed creation without branching on concrete type names. The Repository continues to own file containment, byte limits, Revision ancestry, guarded publication, ChangeSet authorization, and crash recovery; a type definition never performs filesystem or SQLite I/O. History schema version three records the target asset type with every ChangeSet so replay validates durable operations through the same registered definition without guessing from a mutable current file.
...
## PR9 project-level book guidance slice

The planning Asset contribution registers two additional freeform Markdown types: `book.brief` and `book.style-profile`. Both are parentless, stored under the declared `planning` content root, and declare the generic `projectSingleton` contract. The brief records author-confirmed premise, reader promise, protagonist, core conflict, world/canon boundaries, and long-arc direction. The style profile records author-confirmed narrative voice, sentence rhythm, dialogue, information release, serial pacing, hooks, positive examples, and explicit avoidances. Neither body has required fields or a template schema.

The Explorer projects the two types into one Book group, offers human creation only while the exact singleton is absent, and uses registered freeform renderers for title/body saves, exact selections, and text Diffs. `novel_list` automatically advertises both creation contracts, `novel_create` uses the ordinary repository-owned safe path, `novel_get` returns an exact retained body and Revision, and `novel_propose_changes` stays proposal-only. No new model tool schema or Remote method is added.

Book guidance is task-scoped, not hidden ambient prompt state. The Preset persona and relevant PR8 Skills inspect the ordinary `novel_list` catalog, then explicitly `novel_get` a present brief or style profile by its current canonical Revision for matching outline, drafting, rewrite, dialogue, scene, or audit work. Tool calls/results are therefore reconstructable in Session history and absent guidance falls back to the user's current request rather than fabricated project rules. An observation from one draft is never written back automatically.

PR9 does not define finalization or self-evolution. A future slice may let the author explicitly mark one Revision final and compare its draft ancestry plus prompting evidence, but only that explicit finalization may authorize a learning candidate. Trope/reference libraries and AI-flavor scripts are also outside this slice.
...
The input-dock coordinate bar automatically follows the exact saved Revision of whichever registered Asset is currently visible; switching from a chapter to a Book Outline, Volume Outline, or guidance Asset replaces that coordinate. Follow and searched pinned entries are coordinate-only in both the disclosure and model Manifest and require `novel_get` before prose use. An explicit UTF-16 selection remains deliberately simple: its canonical coordinate and complete selected text are frozen together. Version one does not introduce block ids, hidden full-Asset injection, or UI text as authority.

Reader presentation is an optional Client renderer capability, not a `manuscript.chapter` branch in the shared canvas. A renderer may supply authored-character counting and a paper surface; the shared bottom bar spans the complete workbench and always presents coordinated skin, typeface, and font-size controls. Only a chapter adds character count and the chapter-outline action. The paper fills the canvas between the header and bottom bar, and the workbench viewport—not the editable paper—owns vertical scrolling. Project/chapter breadcrumbs and the editable authored title replace technical type/path metadata on the writing surface. The explorer renders Book-level Manuscript and Outline branches even when empty and may collapse independently. View preferences, explorer visibility, and the Agent/workbench split width are client-only state and never enter Frontmatter, Revision history, Context Manifest, system prompts, or tool schemas. The panel boundary is an accessible pointer and keyboard separator with bounded widths; its current value may be discarded without changing authored content.
...
- Additional asset types beyond the shipped chapter, freeform book/volume outline, chapter outline, book brief, and book style profile, including characters, ideas, scenes, timelines, relations, and view definitions.
- Explicit Revision finalization, draft/final learning candidates, preference promotion, trope/reference libraries, and deterministic AI-flavor analysis.
...
- A project scan identifies `manuscript.chapter`, `planning.outline`, `planning.chapter-outline`, `book.brief`, and `book.style-profile` Markdown through their exact declarations, preserves identity across rename, rejects duplicate ids, invalid parent relationships, cycles, per-parent or project-singleton cardinality violations, and escaped paths, reports malformed declarations without rewriting them, and rebuilds every derived catalog value from files.
