...
PR4 不增加另一种作者资产。最初的 `planning.outline` 实现验证了注册 API 可以在不扩大共享 Service 的前提下增加第二套解析器、选区、operation、模型投影、编辑器和 Diff。后续产品验证替换了固定 payload，但保留了同一 Registry 边界。

## PR5 大纲 Registry 验证与当前自由规划切片

自由策划决策[取代了固定大纲 payload 与字段编辑器](../feature/2026-08-23-freeform-outline-and-chapter-plan-assets.zh.md)。此前的严格 YAML 树只保留 Registry 验证意义，不再是受支持的当前磁盘格式。

独立的 Host 与 Client 双端规划类型包现在向 PR4 Registry 注册两种自由 Markdown 类型：`planning.outline` 与 `planning.chapter-outline`。本地 Repository、Remote 方法、上下文解析器、模型工具和共享画布保持类型无关。Explorer 贡献书纲与卷纲的产品导航，正文 Renderer 则在右侧抽屉打开对应章纲。显式 Novel Studio bundle 会在本地 Provider 之前加载 Host 贡献，并在工作台 Renderer Service 可用之后激活 Client 贡献。默认 `web` 与 `headless` 组合保持不变。

`planning.outline` Asset 拥有自由 Markdown 正文，结构上只约束一个 `novel.level`。书纲使用 `level: book` 且没有父级；卷纲使用 `level: volume` 且拥有一个书纲父级。Repository 校验这一语义关系并拒绝更深层级与环，但正文中的标题、散文、列表、表格、节拍记法和自定义约定都由作者决定。`planning.chapter-outline` 是另一种自由 Markdown Asset，必须拥有一个 `manuscript.chapter` 父级，并且每章至多一个。可选章纲方法只会插入为可编辑的起步文本，绝不是必填字段。
...
输入框上方的坐标栏自动跟随当前可见、已保存的注册 Asset 精确 Revision；从章节切换到大纲、卷纲或全书指导 Asset 时会替换该坐标。跟随项与检索固定项在用户披露和模型 Manifest 中都只携带坐标，使用正文前必须 `novel_get`。显式 UTF-16 划词保持刻意简单：规范坐标与完整选中文字一起冻结。第一版不引入 block id、不隐藏注入整 Asset，也不把 UI 文本当作权威。

阅读展示是可选 Client Renderer 能力，而不是共享画布中针对 `manuscript.chapter` 的分支。Renderer 可以提供作者字符统计与纸张表面；共享底栏跨越完整工作台，并始终提供画布/纸张/文字联动皮肤、字体和字号控件。只有正文额外显示本章字数与章纲入口。纸张填满头部与底栏之间的画布，纵向滚动属于工作台视口而不是可编辑纸张。项目/章节面包屑与可编辑的作者标题取代写作表面的技术类型和路径元数据。资产浏览器始终呈现书籍级“正文”和“大纲”分支（包括空分支），并可独立收起。视图偏好、资产栏可见性以及 Agent/工作台分栏宽度都只是客户端状态，绝不进入 Frontmatter、Revision 历史、Context Manifest、system prompt 或工具 Schema。面板边界是同时支持指针和键盘、宽度有界的无障碍 separator；丢弃当前值不会改变作者内容。
...
| 当前类型化 Asset 内容与作者元数据 | Asset 文件和 Frontmatter | 已解析 `AssetSnapshot` | 重新读取精确字节；绝不从索引重建当前作者内容 |
