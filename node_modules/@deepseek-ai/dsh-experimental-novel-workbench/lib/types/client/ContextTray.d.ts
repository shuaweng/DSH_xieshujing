...
import type { NovelAssetSearchResult, NovelContextWorksetDescriptor, SearchNovelAssetsRequest } from '@deepseek-ai/dsh-experimental-novel-repository-remote/types';
...
type ContextTrayProps = PropsRuntime<'conversation.input.dock'> & PropsLocale<'novel-workbench'> & InjectFace<ContextTrayInjected>;
/** Human-visible controls for exact follow/pin references; model payload stays in the Session Log. */
export declare function ContextTray({ sessionId, useSessions, useProjection, useContextFocus, useLibraryContext, useProjectStatus, search, replace, t, }: ContextTrayProps): import("react").JSX.Element | null;
/** Compact human coordinate; the model manifest also carries the canonical dsh-novel URI. */
export declare function contextCoordinate(item: NovelContextWorksetDescriptor['items'][number]): string;
/** Keep the tray readable while the exact coordinate remains available to the model and tooltip. */
export declare function humanContextLabel(label: string): string;
export {};
//# sourceMappingURL=ContextTray.d.ts.map