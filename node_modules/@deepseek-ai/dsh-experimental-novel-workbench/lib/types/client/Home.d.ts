...
import type { NovelAssetDescriptor, NovelProjectDescriptor } from '@deepseek-ai/dsh-experimental-novel-repository-remote/types';
...
export interface NovelHomeInjected {
    readonly inspectLibrary: (candidates: readonly NovelLibraryCandidate[], dayStart: string) => Promise<readonly NovelLibraryBook[]>;
...
type NovelHomeProps = Pick<PropsRuntime<'novel.canvas'>, 'useSessions' | 'useWorkspaces'> & PropsLocale<'novel-workbench'> & NovelHomeInjected & {
    readonly currentProjectId?: NovelProjectDescriptor['id'];
};
/** Minimal working entry: three facts, one next action, and the registered library. */
export declare function NovelHome({ useSessions, useWorkspaces, inspectLibrary, openBook, startNewBook, reportLibraryContext, currentProjectId, t, }: NovelHomeProps): import("react").JSX.Element;
export {};
