---
description: "Model-facing Novel tools for locating exact Assets, creating proposals, reading analysis, and presenting workbench results."
...
- `novel_choose_scene_action` is a bounded gate for key or genuinely uncertain scenes, not a required prelude to ordinary writing. It accepts exactly two or three short dramatic actions. `user` mode asks through the existing `ctx.userQuestions` surface; `agent` mode records the Agent's comparison. The most recent still-active `chapter-execution` or `scene-drive` Skill may be reused from an earlier turn in the same Session. For an existing chapter target, the Host refreshes a precise `chapter-write` Context Manifest from the supplied Asset Revision and defers that model-visible frame after the tool result. Either successful path produces replayable metadata bound to that writing Skill, refreshed Manifest, Project, and exact target Revision.
