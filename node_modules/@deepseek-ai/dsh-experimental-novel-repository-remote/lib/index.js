...
import { isSessionSkillEnabled, replaceSkillActivationSettings } from "@deepseek-ai/dsh-tool-skill";
	...
	return class NovelRepositoryRemote extends _classSuper {
		...
		*/
		async skills(agent, signal) {
			...
			const descriptor = {
				version: 1,
				skills: snapshot.skills.filter((skill) => skill.source === "custom" && (isModelInvocable(skill) || isUserInvocable(skill))).map((skill) => ({
					name: skill.name,
