...
import { NovelContextError } from "./error.js";
import { encodeNovelReferenceUri, parseNovelReferenceText } from "./uri.js";
export { NovelContextError } from "./error.js";
export { NOVEL_REFERENCE_SCHEME, decodeNovelReferenceUri, encodeNovelReferenceUri, formatNovelReferenceMention, parseNovelReferenceText, } from "./uri.js";
...
function normalizePolicies(values) {
    ...
    for (const value of values) {
        if (typeof value !== 'string' || !CONTEXT_POLICIES.has(value)) {
...
function dedupeCandidates(values) {
    ...
    for (const value of [...values].sort((left, right) => right.priority - left.priority)) {
        ...
        if (!value.required && result.some(existing => existing.required
            && existing.assetId === value.assetId && existing.revisionId === value.revisionId))
            continue;
        exact.add(key);
        result.push(value);
    }
...
export class NovelContextResolver extends Service {
    ...
    constructor(ctx, config = {}) {
        ...
        for (const [name, value] of Object.entries(this.config)) {
            if (!Number.isSafeInteger(value) || value <= 0) {
                throw new NovelContextError(`novel context: ${name} must be a positive safe integer`, 'NOVEL_CONTEXT_INVALID_CONFIG');
            }
        }
        ...
        ctx.inject(['sessionProjections'], (projectionCtx) => {
            const schema = zod.custom(value => value === null || isWorkset(value));
    ...
    async replaceWorkset(agent, workset, signal) {
        ...
        for (const item of normalizedItems) {
            await this.ctx.novelRepository.readAsset(project, item.assetId, item.mode === 'pinned' ? item.revisionId : undefined, signal);
        }
        const current = foldNovelContextWorkset(agent.session.events);
        if (current?.version === 2 && JSON.stringify(current) === JSON.stringify(normalized))
    ...
    async resolveReferences(agent, references, signal) {
        ...
        if (first === undefined)
            throw new Error('novel context normalization lost a non-empty reference set');
    ...
    async compile(agent, request, signal) {
        ...
        if (workset !== null) {
            for (const item of workset.items) {
                if (targetAssets.has(item.assetId))
                    continue;
                const snapshot = await this.ctx.novelRepository.readAsset(project, item.assetId, item.mode === 'pinned' ? item.revisionId : undefined, signal);
    ...
    async expandPolicies(project, catalog, candidates, policies, signal) {
        if (policies.length === 1 && policies[0] === 'direct-turn')
            return;
        ...
        for (const target of initial) {
            ...
            if (type === 'planning.chapter-outline' && policies.includes('outline-edit')) {
                ...
                if (snapshot.asset.parentId !== undefined) {
                    addSummary(catalog.find(summary => summary.asset.id === snapshot.asset.parentId), 'coordinate', 'outline-parent', 25);
                }
            }
        }
    }
    async prepareSkillContinuation(agent, messages, turn, step, signal) {
        ...
        if (previous === undefined || previous.references.length === 0)
            return [...messages];
    ...
    async prepareDirectMessages(agent, messages, signal) {
        ...
        const parsed = messages.map((message, index) => {
            ...
            const content = message.content.map((block) => {
                ...
                for (const match of block.text.matchAll(/(^|\s)\/([a-z0-9]+(?:-[a-z0-9]+)*)(?=\s|$)/gu)) {
                    const name = match[2];
                    if (name !== undefined && !invokedSkills.includes(name))
        ...
        if (references.length === 0 && workset === null)
            return parsed;
    ...
    assertSessionBinding(agent, projectId) {
        for (const event of agent.session.events) {
            if (event.type !== 'user/message' || event.data.source.kind !== 'novel-context')
                continue;
            if (event.data.source.projectId !== projectId) {
                throw new NovelContextError('novel context: Session is already bound to another Novel Project', 'NOVEL_CONTEXT_SESSION_BOUND');
            }
        }
    }
}
...
function isWorkset(value) {
    ...
    if ((record['version'] !== 1 && record['version'] !== 2)
        || typeof record['projectId'] !== 'string' || !Array.isArray(record['items']))
        return false;
    let follows = 0;
    const valid = record['items'].every((item) => {
        ...
        if (entry['projectId'] !== record['projectId'] || typeof entry['assetId'] !== 'string'
            ...
            return false;
        if (record['version'] === 1) {
            if (!isReference(item) || (entry['mode'] !== 'follow' && entry['mode'] !== 'pinned'))
                return false;
...
function isContextSurface(value) {
    ...
    if (record['kind'] !== 'library-home'
        ...
        || !Array.isArray(record['books']) || record['books'].length > MAX_LIBRARY_BOOKS)
        return false;
 ...
 */
export function foldNovelContextWorkset(events) {
    let current = null;
    for (const event of events) {
        if (event.type !== 'novel/context-workset')
            continue;
        const data = event.data;
        if (isWorkset(data.workset))
            current = data.workset;
    }
    return current;
}
...
function loadedSkillBeforeStep(events, turn, step) {
    ...
    for (const event of events) {
        if (event.type === 'tool/call') {
            ...
            if (data.turn !== turn || data.step !== step - 1 || data.name !== 'skill'
                || typeof data.callId !== 'string' || typeof data.arguments !== 'string')
                continue;
            try {
                const args = JSON.parse(data.arguments);
                if (typeof args.name === 'string')
                    calls.set(data.callId, args.name);
            }
            catch {
                continue;
            }
    ...
    for (const [callId, name] of [...calls].reverse())
...
function latestTurnManifest(events, turn) {
    ...
    for (const event of events) {
        ...
        if (!inTurn || event.type !== 'user/message')
            continue;
...
function materializedInTurn(events, turn, target) {
    ...
    for (const event of events) {
        ...
        if (manifest.references.some(reference => reference.assetId === target.assetId
            ...
            && selectorIdentity(reference.selector) === selectorIdentity(target.selector)))
            return true;
    }
    return false;
}
function readSkillContextPolicy(metadata) {
    ...
    return typeof value === 'string' && CONTEXT_POLICIES.has(value)
        ? value
        : undefined;
}
...
function stringifyTagSafeJson(value) {
    ...
    if (typeof text !== 'string')
        throw new TypeError('novel context is not JSON-serializable');
