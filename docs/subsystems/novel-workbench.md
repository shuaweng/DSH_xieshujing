...
[`@deepseek-ai/dsh-experimental-tool-novel`](../../packages/experimental/tool-novel) exposes approval-gated `novel_initialize_project`, `novel_list`, `novel_search`, `novel_create`, `novel_get`, explicit read-only `novel_get_analysis`, bounded `novel_choose_scene_action`, `novel_propose_changes`, and presentation-only `novel_present` in the package-owned Preset. Persisted analysis remains outside ordinary context and authored-Asset discovery; the Agent reads it only for an exact chapter Revision when requested. Initialization and the browser empty state call the same Repository operation: it preserves existing files, creates the minimal content roots, and publishes `novel.yaml` last as the activation marker; a present invalid manifest or conflicting content-root path fails without replacement. Catalog discovery returns canonical exact-Revision references and registered creation contracts. A new chapter is one `novel_create` call containing its title and complete manuscript body; the Agent never requires a separately created empty container. If an empty chapter already exists, one proposal can combine `update-title` with `insert-text` at UTF-16 offset zero, while non-empty rewrites use `replace-text`. The registered type serializes the title and body once into one Revision. Exact reads use each type's deterministic model projection and proposal instructions. Fixed workflows receive compiler-frozen related material, while exploratory reads remain explicit tool calls retained in Session history. A proposal uses the matching definition to validate exact type-owned operations before durably creating a ChangeSet without changing the authored file. `novel_present` only opens or closes the whole workbench through typed tool-result metadata. The model has no apply tool and cannot claim publication.

Scene execution remains a Skill-directed branch inside the native Agent loop. Ordinary scenes go directly to creation or proposal. For a key or genuinely uncertain scene, `chapter-execution` or `scene-drive` may submit exactly two or three short dramatic actions to `novel_choose_scene_action`. Author-owned selection reuses `ctx.userQuestions`, so the ordinary Composer takeover displays and resumes the choice; Agent-owned selection completes without a human wait. In both modes the successful tool call and result are durable Session events. The final creation/proposal cites that call id, and the Host accepts it only in the same Session turn, Context Manifest, writing Skill, Project, and exact target Revision. Generation lineage retains the decision call id plus derived coordinates, while the option prose remains only in the Session event. No Scene database, permanent Scene Contract Asset, separate context store, or novel-specific interaction state machine is introduced.
