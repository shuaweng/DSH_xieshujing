...
| `@deepseek-ai/dsh-experimental-tool-novel` | `novel_get`、`novel_list`、`novel_propose_changes` | `ctx.tools`、`ctx.systemPrompt`、`ctx.novelContextResolver`、`ctx.novelRepository`、`ctx.fs`、`ctx.sandboxPolicy`、`an owning Agent Session at execution time` | `tool/call`、`durable proposal-only ChangeSet from novel_propose_changes`、`tool/result` | - | Novel Studio Preset 提供 3 个稳定工具，但不提供通用文件系统修改能力。`novel_list` 发现规范的当前章节引用，`novel_get` 解析精确保留 Revision，`novel_propose_changes` 只创建可审阅 ChangeSet，绝不发布作者文件。 |
...
列出当前 Session 的 Novel Project 及其章节 Asset，并返回规范的精确 Revision 引用。
