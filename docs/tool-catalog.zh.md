...
| `@deepseek-ai/dsh-experimental-tool-novel` | `novel_get`、`novel_propose_changes` | `ctx.tools`、`ctx.systemPrompt`、`ctx.novelContextResolver`、`ctx.novelRepository`、`an owning Agent Session at execution time` | `tool/call`、`durable proposal-only ChangeSet from novel_propose_changes`、`tool/result` | - | Novel Studio Preset 提供这两个稳定工具，但不提供通用文件系统修改能力。`novel_get` 解析精确保留 Revision；`novel_propose_changes` 只创建可审阅 ChangeSet，绝不发布作者文件。 |
...
Novel Studio Preset 提供这两个稳定工具，但不提供通用文件系统修改能力。`novel_get` 解析精确保留 Revision；`novel_propose_changes` 只创建可审阅 ChangeSet，绝不发布作者文件。
